...
# Define a mutation matrix based on the Kimura substitution model for nucleotides (A, C, G, T).
kimura_matrix = {
...
# Define function for aplly mutation

# Introduce subtitutions
...
# Introduces mutations (substitutions, insertions, deletions) in the sequence, along with sequencing errors.
    ...
    for i in range(seq_length):
        if random.random() < mutation_rate:
            mutation_type = random.choices(
                ...
                [0.8, 0.1, 0.1], # Probabilities for substitution, insertion, and deletion.
                k=1
            )[0]
...
class SimulateReads:
    ...
    # Simulates reads based on input parameters and writes the output to FASTQ files.
    def simulate_reads(self):
        fasta = ps.FastaFile(self.genome_fasta) # Load the genome FASTA file using pysam.
        all_reads = []
        ...
        # Process for simulating short reads
        if self.sequence == 'short':
            # Loop through each entry in the BED file, simulating reads for each circular molecule.
            for circle_info in self.circle_bed:
                ...
                for _ in range(round(reads)):
                    ...
                    # Simulate left and right paired-end reads
    ...
    # Generate a quality score line for FASTQ files based on the specified Phred score.
    def generate_quality_line(self, phred_score, length):
        return chr(phred_score + 33) * length

    # Writes simulated reads to FASTQ files.
    def write_fastq_files(self, file_path, all_reads, sequence, phred_score=40):
        if sequence == 'short':
            ...
            # Write R2 records to the R1 file
    ...
    # Main function that runs the simulation by reading the BED file and generating reads.
    def run(self):
